...
#' @export
#'
indikatorFigAndelGrVar2017 <- function(AntTilfeller, N, outfile, tittel, width=800, height=700, sideTxt='Boområde/opptaksområde',
                                   ...
                                   minstekravTxt='Min.', maalTxt='Mål', graaUt=NA)
{
  ...
  if (dim(andeler)[2]==2) {
    mtext( c(N[,1], names(N)[1]), side=4, line=2.5, las=1, at=c(ypos, max(ypos)+diff(ypos)[1]), col=1, cex=cexgr, adj = 1)
    mtext( c(N[,2], names(N)[2]), side=4, line=5.5, las=1, at=c(ypos, max(ypos)+diff(ypos)[1]), col=1, cex=cexgr, adj = 1)
    ...
    mtext( c(N[,1], names(N)[1]), side=4, line=2.5, las=1, at=c(ypos, max(ypos)+diff(ypos)[1]), col=1, cex=cexgr, adj = 1)
    mtext( c(N[,2], names(N)[2]), side=4, line=5, las=1, at=c(ypos, max(ypos)+diff(ypos)[1]), col=1, cex=cexgr, adj = 1)
    mtext( c(N[,3], names(N)[3]), side=4, line=7.5, las=1, at=c(ypos, max(ypos)+diff(ypos)[1]), col=1, cex=cexgr, adj = 1)
